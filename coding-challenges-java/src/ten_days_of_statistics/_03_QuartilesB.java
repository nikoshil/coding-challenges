...
public class _03_QuartilesB {
    ...
    public static List<Integer> calculate(int n, List<Integer> list) {
        List<Integer> lowerHalf = list.subList(0, n / 2);
        List<Integer> upperHalf;

        if (n % 2 != 0)
            upperHalf = list.subList(n / 2 + 1, n);
        else
            upperHalf = list.subList(n / 2, n);

        return Arrays.asList(
                median(lowerHalf.size(), lowerHalf),
                median(n, list),
                median(upperHalf.size(), upperHalf)
        );
    }

    private static int median(int n, List<Integer> list) {
        if (n % 2 == 0) {
